...
class AVFDetectorApp:
    ...
    def __init__(self):
        ...
        self.cpu_monitor_interval = 0.5 # Faster polling for peak detection
    ...
    def create_file_scanner_screen(self):
        ...
        self.analyze_btn = tk.Button(
            ...
            bg='#27ae60', fg='white', command=self.record_analyze_file, cursor="hand2", width=14
        )
    ...
    def start_recording(self):
        ...
        if self.mic_device_id is None:
            messagebox.showerror("No Microphone", "No microphone detected.")
            return
        
        self.is_recording = True
        self.analyze_btn.config(text="Stop Recording", bg='#e74c3c')
        self.file_result_frame.config(bg='#27ae60')
        self.file_result_label.config(text="RECORDING", bg='#27ae60')
        self._update_scroll_text("Recording Acoustic Vibration...\nPlease wait 30 seconds.")
    ...
    def _record_audio(self):
        try:
            ...
            temp_file = recordings_dir / f"recording_{datetime.now().strftime('%Y%m%d_%H%M%S')}.wav"
            sf.write(str(temp_file), self.recording_data, self.mic_sample_rate)
            self.selected_file = str(temp_file)
            
            self.window.after(0, self._start_analysis_phase)
    ...
    def _run_file_analysis(self):
        try:
            ...
            # UI Update
            self.window.after(0, lambda: self._finalize_ui(prediction, stats, proc_time_ms))

        except Exception as e:
            self.is_analyzing = False
            self.window.after(0, lambda: messagebox.showerror("Error", f"Analysis failed: {e}"))

    def _finalize_ui(self, prediction, stats, proc_time):
        color = '#c0392b' if prediction == 'STENOSIS' else '#27ae60'
        self.file_result_frame.config(bg=color)
        self.file_result_label.config(text=prediction, bg=color)
